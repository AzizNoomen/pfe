...
async def create_document(title: str, content: str, session: Session = Depends(get_db_session)):
    logger.info("Creating a document with title: %s", title)
    document_service = DocumentService(session)
    result = document_service.create_document(title, content)
    logger.info("Document created with ID: %s", result.id)
    return result
